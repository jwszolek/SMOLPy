...
def _draw_topology(
    ...
    for a, b in graph.edges():
        ...
        dpg.draw_line([ax, ay], [bx, by],
                      color=list(_LINK_COLOR), thickness=2.5,
                      parent="topo_canvas")
    ...
    for name, (cx, cy) in pos.items():
        ...
        dpg.draw_text([label_x, cy + _NODE_R + 6], name,
...
def _run_dashboard(
    ...
    with dpg.window(tag="main", no_title_bar=True, no_move=True,
        ...
        with dpg.group(horizontal=True):
            dpg.add_text(f"  PySMOL  ·  {network.name}  ·  {duration_ms:,.0f} ms",
                         color=[110, 190, 255, 255])
            dpg.add_spacer(width=20)
            dpg.add_text("● Simulating…" if live_mode else "● Done",
                         tag="status_text",
        ...
        with dpg.group(horizontal=True):
            ...
            with dpg.group():
                ...
                with dpg.child_window(width=r_w, height=r_h,
                                      border=False, tag="metrics_panel"):
                    for idx, (metric_name, targets) in enumerate(metric_groups.items()):
                        ...
                        with dpg.plot(label=label, height=plot_h_init,
                                      width=-1, tag=plot_tag):
                            dpg.add_plot_legend()
    ...
    while dpg.is_dearpygui_running():
        ...
        for name in network._nodes:
            ...
            if name in receiving and name not in sending:
                ...
                dpg.configure_item(f"_nr_{name}",
                                   color=[*_RECEIVER_COLOR, ring_a])
            else:
                dpg.configure_item(f"_nr_{name}",
                                   color=[*_RECEIVER_COLOR, 0])
        ...
        for a, b in links:
            ...
            for i in range(_DOTS_PER_LINK):
                ...
                dpg.configure_item(dot_tags[dot_idx],
                                   center=[ax + phase * (bx - ax),
                                           ay + phase * (by - ay)])
                dot_idx += 1
                phase = (now * _SPEED + (i + 0.5) / _DOTS_PER_LINK) % 1.0
                dpg.configure_item(dot_tags[dot_idx],
                                   center=[bx + phase * (ax - bx),
                                           by + phase * (ay - by)])
                dot_idx += 1
